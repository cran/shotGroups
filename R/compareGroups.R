...
function(DF, plots=TRUE, xyTopLeft=TRUE, ABalt=c("two.sided", "less", "greater"),
    ...
    distPOAmoa  <- getMOA(distPOA, dst=dstTarget, conversion=conversion)
    ...
    ## sd and sd CIs as separate lists for unit of measurement and MOA
    sdXYunit   <- lapply(xyL, function(x) sqrt(diag(cov(x)))) # standard deviations
    sdXYmoa    <- Map(getMOA, sdXYunit,   dst=dstTarget, conversion=conversion)
    sdXYciUnit <- lapply(xyL, getSDxyCI)             # confidence intervals
    sdXYciMOA  <- Map(getMOA, sdXYciUnit, dst=dstTarget, conversion=conversion)

    ## add to results list as matrices
    res$sdXY      <- do.call("cbind", sdXYunit)
    res$sdXYmoa   <- do.call("cbind", sdXYmoa)
    res$sdXYci    <- do.call("cbind", sdXYciUnit)
    res$sdXYciMOA <- do.call("cbind", sdXYciMOA)

    ## mean distances to group center
    meanDstCtr    <- sapply(xyL, function(x) mean(getDistToCtr(x)))
    meanDstCtrMOA <- Map(getMOA, meanDstCtr, dst=dstTarget, conversion=conversion) # MOA
    res$meanDistToCtr <- rbind(unit=meanDstCtr, MOA=meanDstCtrMOA)
    ...
    maxSpreadMOA <- getMOA(maxSpread, dst=dstTarget, conversion=conversion)
    ...
    bbFoM      <- sapply(bbs, function(x) { x$FoM } )
    bbDiag     <- sapply(bbs, function(x) { x$diag } )
    bbFoMMOA   <- getMOA(bbFoM,  dst=dstTarget, conversion=conversion)
    bbDiagMOA  <- getMOA(bbDiag, dst=dstTarget, conversion=conversion)
    res$bbFoM  <- rbind(unit=bbFoM,  MOA=bbFoMMOA)
    res$bbDiag <- rbind(unit=bbDiag, MOA=bbDiagMOA)

    ## radius of minimum enclosing circle
    minCircs         <- lapply(xyL, getMinCircle)
    minCircleRad     <- sapply(minCircs, function(x) { x$rad } )     # radius
    minCircleRadMOA  <- getMOA(minCircleRad,
                               dst=dstTarget, conversion=conversion) # as MOA
    res$minCircleRad <- rbind(unit=minCircleRad, MOA=minCircleRadMOA)

    ## 50% circular error probable
    CEPlist <- Map(getCEP, xyL, dstTarget=dstTarget, conversion=conversion,
                   type="Rayleigh", accuracy=FALSE)
    CEPrayL <- lapply(CEPlist, function(x) { x$Rayleigh[ , "50%", drop=FALSE] } )
    CEPmat  <- do.call("cbind", CEPrayL)     # as matrix
    ...
    if(nS == 2) {                        # compare two groups
        ...
        res$Kruskal  <- coin::kruskal_test(dstCtr ~ Series,    # dist to center
                                           data=dstCtrDf, distribution="approximate")
    }
    ...
    if(plots) {
        ...
        plot(Y ~ X, data=DF, asp=1, lwd=2, xlab="X", ylab="Y",
        ...
        devNew()                         # open new diagram
        plot(Y ~ X, data=DF, asp=1, xlim=xLims, ylim=yLims,
             xlab="X", ylab="Y", lwd=2,
             pch=syms[unclass(DF$Series)], col=cols[unclass(DF$Series)],
